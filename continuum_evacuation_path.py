...
#     "exit_color":  (0, 220, 80),
#     "path_color":  (0, 220, 80),
#     "path_alpha":  0.55,         # opacity of each line segment
#     "line_thickness": 1,
# }
# # ══════════════════════════════════════════════════════════════
...
#     # ── Render ───────────────────────────────────────────────
#     print("Rendering…")
#     # Base image from mask (convert to colour)
#     base = cv2.cvtColor(wm.raw, cv2.COLOR_GRAY2BGR)
#     # Walls → grey, walkable → dark
#     out = np.zeros((wm.h, wm.w, 3), dtype=np.uint8)
#     out[:] = CFG["bg_color"]
#     wall_mask = wm.raw >= 128
#     out[wall_mask] = CFG["wall_color"]

#     # Draw paths onto a float accumulation buffer for alpha blending
#     path_layer = np.zeros((wm.h, wm.w, 3), dtype=np.float32)
#     path_count = np.zeros((wm.h, wm.w),    dtype=np.float32)
...
#             cv2.line(path_layer, (x1,y1),(x2,y2), pc, CFG["line_thickness"])
#             cv2.line(path_count, (x1,y1),(x2,y2), 1,  CFG["line_thickness"])

#     # Tone-map: more overlapping paths → brighter green
#     density = np.clip(path_count / 8.0, 0, 1)   # saturate at 8 overlaps
#     density3 = density[:,:,np.newaxis]
#     path_rgb = np.clip(path_layer, 0, 255).astype(np.float32) / 255.0

#     # Composite path layer over background
#     alpha = CFG["path_alpha"] * density3
#     out_f = out.astype(np.float32) / 255.0
#     comp  = out_f*(1-alpha) + path_rgb*alpha
#     result = (np.clip(comp,0,1)*255).astype(np.uint8)

#     # Draw exit circles
#     for ex, ey in exits:
#         cv2.circle(result, (ex, ey), CFG["exit_radius"],
#                    CFG["exit_color"], 2)
#         cv2.putText(result, "EXIT", (ex-18, ey+CFG["exit_radius"]+14),
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.4, CFG["exit_color"], 1)
